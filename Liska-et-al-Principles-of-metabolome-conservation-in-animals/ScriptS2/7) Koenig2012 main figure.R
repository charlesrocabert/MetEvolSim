...
#
# 7) Koenig2012 main figure.R
# ---------------------------
# Create the main figure for the Koenig (2012) model.
#***************************************************************************
...
build_enzyme_essentiality_score_dataset <- function( drop_coefficient, dist_threshold )
{
  ...
  #----------------------------------------#
  sim  = read.table("Koenig2012_simulations.txt", h=T, sep=" ")
  ...
  #----------------------------------------#
  flux_drop    = read.table("Koenig2012_flux_drop_analysis.txt", h=T, sep=" ")
  ...
  flux_to_met  = read.table("Koenig2012_reaction_to_species_map.txt", h=F, sep=" ")
  REACTION     = c()
  METABOLITE   = c()
  MOMA         = c()
  ABUNDANCE    = c()
...
plot_fitness_coupling <- function( D )
{
  ...
  p = ggplot(D, aes(x=Abundance, y=Fitness_coupling)) +
    ...
    geom_point() +
    annotate(geom="text", x=-2.5, y=0.35, label=stat_line, hjust=0, size=3.5) +
    xlab("Metabolite abundance (log-scale)") +
...
plot_fitness_coupling_distribution <- function( filename, title )
{
  ...
  p = ggplot(coupling, aes(Rho)) +
    ...
    annotate(geom="label", x=coupling$Rho[[1]]-0.1, y=1700, label=Target, fill="white") +
...
plot_simulation_dataset <- function( exploration, nb_evals, index_max, outliers, sens, title )
{
  ...
  for(i in seq(1, nb_evals))
  {
    ...
    try({
      res       = compute_correlation(paste("Koenig2012_fitness_coupling_simulations/statistics_",i,".txt", sep=""), index_max, outliers, sens, target)
      print(res)
      Rho       = c(Rho, rho)
  ...
  p = ggplot(dataset, aes(x=Rho, y=EvoRho)) +
    annotate(geom="text", x=0.11, y=-0.1, label=stat_line, hjust=0, size=3.5) +
    geom_point() +
...
# 4) Create the figure    #
#-------------------------#
p1 = plot_abund_CS(select, "Stabilizing selection", y_range1, -5.8, 7.5)
p2 = plot_abund_CS(drift, "Genetic drift", y_range1, -5.5, 7.5)
...
p6 = plot_fitness_coupling_distribution("Koenig2012_fitness_coupling_exploration.txt", "Exploration of 10,000 random\ncombinations of target fluxes")

nb_evals    = 100
exploration = read.table("Koenig2012_fitness_coupling_exploration.txt", sep=" ", h=T)
...
ggsave("Koenig2012_main_figure.png", p, dpi=600, bg="white", scale=3.5, width=3.5, height=3)
